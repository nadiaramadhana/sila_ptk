<x-layouts.app>
    <div class="container-fluid">
        ...
        {{-- Info Pengajuan --}}
        <div class="card mb-4">
            <div class="card-header text-white fw-semibold d-flex align-items-center gap-2"
        ...
        @if ($detail)
            <div class="card mb-4">
                ...
                <div class="card-body">
                    @include('dashboard.pengajuan.partials.show_' . str_replace('-', '_', $slug), [
                        'detail' => $detail,
                    ])
                </div>
            </div>
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('pengajuan.update-status', $pengajuan) }}">
                        ...
                        <div class="row g-3 align-items-end">
                            <!-- 1. Dropdown Pilihan Status -->
                            <div class="col-md-6">
                                <label class="form-label">Status Pengajuan</label>
                                <select name="status" id="statusPengajuan" class="form-select">
                            ...
                            <!-- 2. Box Catatan (Akan muncul jika status diproses, selesai, atau ditolak) -->
                            <div class="col-md-6" id="catatanWrap"
                                style="{{ in_array($pengajuan->status, ['diproses', 'selesai', 'ditolak']) ? '' : 'display:none' }}">
                            ...
                            <div class="col-md-2">
                                <button type="submit" class="btn w-100 text-white" style="background-color: #0f1f3d;">
                                    Simpan
                                </button>
                            </div>
                        </div>
        ...
        <div class="d-flex gap-2">
            @if (in_array($pengajuan->status, ['draft', 'ditolak']))
                ...
                <form action="{{ route('pengajuan.destroy', $pengajuan) }}" method="POST" class="js-delete-form"
    ...
    @push('scripts')
        <script>
            document.getElementById('statusPengajuan')?.addEventListener('change', function() {
                const showCatatan = ['diproses', 'selesai', 'ditolak'].includes(this.value);
                document.getElementById('catatanWrap').style.display = showCatatan ? '' : 'none';
            })
        </script>
    @endpush
</x-layouts.app>
